...
- (instancetype)initWithMessage:(EMMessage *)message photo:(UIImage *)userPhoto
{
    self = [super init];
    if (self) {
        _cellHeight = -1;
        ...
        if (_isSender)
        {
            _avatarImage = userPhoto;
        }
        else
        {
            _avatarImage = [UIImage imageNamed:@"headerImage_doctor"];
        }
        ...
        switch (_firstMessageBody.type) {
            case EMMessageBodyTypeText:
            {
                ...
                if ([message.ext objectForKey:@"askCount"]) {
                    _askCount = [message.ext objectForKey:@"askCount"];
                }
            }
                break;
