...
-(void)reduceIntensityClick:(UIButton *)sender
{
    ...
    {
        alert = [[UIAlertView alloc] initWithTitle:nil message:@"Please connect to Somnormal" delegate:self cancelButtonTitle:nil otherButtonTitles:nil];
        [NSTimer scheduledTimerWithTimeInterval:2.0f target:self selector:@selector(performDismissAtCES:) userInfo:nil repeats:NO];
        [alert show];
    }
}
...
-(void)addSectionFour
{
    ...
    if (SCREENWIDTH==414)
    {
        optionButton.frame=CGRectMake(SCREENWIDTH*2/7, CES_SCREENH_HEIGHT/10+CES_SCREENH_HEIGHT/2, SCREENWIDTH*3/7, SCREENWIDTH*3/7);
        anotherOptionButton.frame=CGRectMake(SCREENWIDTH*2.5/7, CES_SCREENH_HEIGHT/10+CES_SCREENH_HEIGHT*6.7/10, SCREENWIDTH*2/7, SCREENWIDTH/7);
    }
    else
    {
        optionButton.frame=CGRectMake(SCREENWIDTH*2/7, CES_SCREENH_HEIGHT/10+CES_SCREENH_HEIGHT/2, SCREENWIDTH*3/7, SCREENWIDTH*3/7);
    ...
    if (_bluetoothInfo!=nil)
    {
        ...
        [optionButton setTitle:@"Unconnected" forState:UIControlStateNormal];
        optionButton.tag=11;
    ...
    {
        [optionButton setTitle:@"Unbound" forState:UIControlStateNormal];
    }
    optionButton.backgroundColor=[UIColor whiteColor];
    optionButton.layer.cornerRadius=SCREENWIDTH*3/14;
    optionButton.tag=0;
...
-(void)doSomething:(UIButton *)sender
{
    ...
    {
        //提示蓝牙未开启
        alert = [[UIAlertView alloc] initWithTitle:nil message:@"Bluetooth not open" delegate:self cancelButtonTitle:nil otherButtonTitles:nil];
        [NSTimer scheduledTimerWithTimeInterval:2.0f target:self selector:@selector(performDismissAtCES:) userInfo:nil repeats:NO];
        [alert show];
    }
    else
    {
        ...
        else if (sender.tag==11)
        {
            ...
            [connectIng setTitle:@"Connecting..." forState:UIControlStateNormal];
            if (SCREENWIDTH==320)
            {
...
-(void)sendStartWork
{
    ...
    if (timeout==time)
    {
        ...
        dispatch_source_set_event_handler(_timer, ^{
                ...
                else if (timeout%60==0)
                {
                    if (timeout==time-60)
                    {
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            ...
                            [anotherOptionButton setTitle:@"Stop" forState:UIControlStateNormal];
                        });
                        timeout--;
                    }
                    else
            ...
            {
                if (![optionButton.titleLabel.text isEqualToString:@"Unconnected"] || ![optionButton.titleLabel.text isEqualToString:@"Unbound"])
                {
                    dispatch_async(dispatch_get_main_queue(), ^{
...
-(void)sendStopOrder
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (_discoveredPeripheral!=nil)
        {
            optionButton.titleLabel.text=str;
            [optionButton setTitle:str forState:UIControlStateNormal];
            [anotherOptionButton setTitle:@"Start" forState:UIControlStateNormal];
        }
        else
        {
            [optionButton setTitle:@"Unbound" forState:UIControlStateNormal];
...
-(void)cancelBlutooth:(NSTimer *)timer
{
    ...
    if (countSeconds>=120)
    {
        ...
        if ([optionButton.titleLabel.text isEqualToString:optionButtonState])
        {
            ...
            //设置代理传值(电量)
            [self.delegate sendElectricQualityValue:@"Unconnected"];
        }
        [timer invalidate];
    }
}
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(nullable NSError *)error
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (![optionButton.titleLabel.text isEqualToString:@"Unconnected"] && ![optionButton.titleLabel.text isEqualToString:@"Unbound"])
        {
            ...
            //删除“正在连接...”按钮
            if ([connectIng.titleLabel.text isEqualToString:@"Connecting..."])
            {
                [connectIng removeFromSuperview];
                //                optionButton.userInteractionEnabled=YES;
                //                anotherOptionButton.userInteractionEnabled=YES;
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateNotificationStateForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    if ([value containsString:@"55bb010a"])
    {
        ...
        if (percent>5 && percent<=20)
        {
            if (countElectric<1)
            {
                alert = [[UIAlertView alloc] initWithTitle:nil message:@"Battery power is less than 20%. Please charge device promptly" delegate:self cancelButtonTitle:nil otherButtonTitles:nil];
                [NSTimer scheduledTimerWithTimeInterval:2.0f target:self selector:@selector(performDismissAtCES:) userInfo:nil repeats:NO];
                [alert show];
        ...
        else if(percent<=5)
        {
            ...
            {
                alert = [[UIAlertView alloc] initWithTitle:nil message:@"Battery power is less than 5%. Device unable to function, please charge device first" delegate:self cancelButtonTitle:nil otherButtonTitles:nil];
                [NSTimer scheduledTimerWithTimeInterval:2.0f target:self selector:@selector(performDismissAtCES:) userInfo:nil repeats:NO];
                [alert show];
                countElectric_Two++;
            }
...
-(void)selectModel:(UIButton *)sender
{
    ...
    if (sender.tag==0)
    {
        [tmp0 setImage: [UIImage imageNamed:@"selected"] forState:UIControlStateNormal];
        [tmp1 setImage: [UIImage imageNamed:@"unselected"] forState:UIControlStateNormal];
        [tmp2 setImage: [UIImage imageNamed:@"unselected"] forState:UIControlStateNormal];
        [modelButton setTitle:@"Mode 1" forState:UIControlStateNormal];
        [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(removeViews) userInfo:nil repeats:NO];
    }
    else if (sender.tag==1)
    {
        [tmp0 setImage: [UIImage imageNamed:@"unselected"] forState:UIControlStateNormal];
        [tmp1 setImage: [UIImage imageNamed:@"selected"] forState:UIControlStateNormal];
        [tmp2 setImage: [UIImage imageNamed:@"unselected"] forState:UIControlStateNormal];
        [modelButton setTitle:@"Mode 2" forState:UIControlStateNormal];
        [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(removeViews) userInfo:nil repeats:NO];
    }
    else if (sender.tag==2)
    {
        [tmp0 setImage: [UIImage imageNamed:@"unselected"] forState:UIControlStateNormal];
        [tmp1 setImage: [UIImage imageNamed:@"unselected"] forState:UIControlStateNormal];
        [tmp2 setImage: [UIImage imageNamed:@"selected"] forState:UIControlStateNormal];
        [modelButton setTitle:@"Mode 3" forState:UIControlStateNormal];
        [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(removeViews) userInfo:nil repeats:NO];
    }
}
