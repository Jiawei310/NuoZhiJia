...
@interface JustinMenuView : UIView

@end

@interface JustinMenuOverlay : UIView

@end

@implementation JustinMenuOverlay
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touched == self)
    {
        ...
        {
            if ([v isKindOfClass:[JustinMenuView class]] && [v respondsToSelector:@selector(dismissMenu:)])
            {
...
@implementation JustinMenuItem

+ (instancetype)menuItem:(NSString *)title image:(UIImage *)image target:(id)target action:(SEL)action
{
    return [[JustinMenuItem alloc] init:title image:image target:target action:action];
}

- (id)init:(NSString *)title image:(UIImage *)image target:(id)target
     action:(SEL) action
{
...
typedef enum {
    ...
    JustinMenuViewArrowDirectionNone,
    JustinMenuViewArrowDirectionUp,
    JustinMenuViewArrowDirectionDown,
    JustinMenuViewArrowDirectionLeft,
    JustinMenuViewArrowDirectionRight,
    
}JustinMenuViewArrowDirection;

@implementation JustinMenuView
{
    JustinMenuViewArrowDirection    _arrowDirection;
...
- (void)setupFrameInView:(UIView *)view fromRect:(CGRect)fromRect
{
    ...
    {
        _arrowDirection = JustinMenuViewArrowDirectionUp;
    ...
    {
        _arrowDirection = JustinMenuViewArrowDirectionDown;
        CGPoint point = (CGPoint){
            rectXM - widthHalf,
    ...
    {
        _arrowDirection = JustinMenuViewArrowDirectionLeft;
        CGPoint point = (CGPoint){
            rectX1,
            rectYM - heightHalf
        };
    ...
    {
        _arrowDirection = JustinMenuViewArrowDirectionRight;
        CGPoint point = (CGPoint){
    ...
    {
        _arrowDirection = JustinMenuViewArrowDirectionNone;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems
{
    ...
    JustinMenuOverlay *overlay = [[JustinMenuOverlay alloc] initWithFrame:view.bounds];
        ...
        if (animated)
        {
            ...
            [UIView animateWithDuration:0.2
                             animations:^(void) {
                                 ...
                                 if ([self.superview isKindOfClass:[JustinMenuOverlay class]])
                                 {
...
- (void)performAction:(id)sender
{
    ...
    JustinMenuItem *menuItem = _menuItems[button.tag];
...
- (UIView *)mkContentView
{
    ...
    UIFont *titleFont = [JustinMenu titleFont];
    if (!titleFont)
    {
    ...
    for (JustinMenuItem *menuItem in _menuItems)
    {
...
//        const CGSize titleSize = [menuItem.title sizeWithFont:titleFont];
        const CGSize titleSize = [menuItem.title sizeWithAttributes:@{NSFontAttributeName : [UIFont fontWithName:titleFont.fontName size:titleFont.pointSize]}];
        const CGSize imageSize = menuItem.image.size;
    ...
    UIImage *selectedImage = [JustinMenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight + 2}];
    UIImage *gradientLine = [JustinMenuView gradientLine: (CGSize){maxItemWidth - kMarginX * 4, 1}];
...
- (CGPoint)arrowPoint
{
    ...
    if (_arrowDirection == JustinMenuViewArrowDirectionUp)
    {
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMinY(self.frame) };
    }
    else if (_arrowDirection == JustinMenuViewArrowDirectionDown)
    {
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMaxY(self.frame) };
    }
    else if (_arrowDirection == JustinMenuViewArrowDirectionLeft)
    {
        point = (CGPoint){ CGRectGetMinX(self.frame), CGRectGetMinY(self.frame) + _arrowPosition  };
    }
    else if (_arrowDirection == JustinMenuViewArrowDirectionRight)
    {
...
- (void)drawBackground:(CGRect)frame inContext:(CGContextRef) context
{
    ...
    UIColor *tintColor = [JustinMenu tintColor];
    if (tintColor)
    {
        CGFloat a;
    ...
    if (_arrowDirection == JustinMenuViewArrowDirectionLeft || _arrowDirection == JustinMenuViewArrowDirectionRight)
    {
        start = (CGPoint){X0, Y0};
        end = (CGPoint){X1, Y0};
    }
...
static JustinMenu *gMenu;
static UIColor *gTintColor;
static UIFont *gTitleFont;

@implementation JustinMenu
{
    JustinMenuView *_menuView;
    BOOL        _observing;
}
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems
{
    ...
    _menuView = [[JustinMenuView alloc] init];
